...
const app = express();
const MONGO_URL = process.env.MONGO_URL || "mongodb://127.0.0.1:27017/disasterDB";
  ...
  try {
    ...
    if (!adminExists) {
      ...
      const newAdmin = new User({
        ...
        email: process.env.DEFAULT_ADMIN_EMAIL || "admin@disastermanage.com",
        userType: "admin",
        name: "System Administrator"
      });
      
      // Use passport-local-mongoose register method to hash password
      await User.register(newAdmin, process.env.DEFAULT_ADMIN_PASSWORD || "admin123");
...
// Add email test route EARLY to avoid conflicts with :id routes
app.get("/admin/email-test", isLoggedIn, isAdmin, async (req, res) => {
  try {
    const mailOptions = {
      from: `"Disaster Management" <${process.env.EMAIL_USERNAME}>`,
      to: req.user.email,
      subject: "Test Email from Disaster Management",
...
// Function for creating a new alert and notifying users
async function createAlert(title, message, reportId, alertType = "new_disaster") {
  try {
    ...
    if (!report) {
      console.error("Report not found for alert:", reportId);
      return newAlert;
    }
    ...
    for (const user of allUsers) {
      ...
      const mailOptions = {
        ...
        html: `
          ...
          <p>Time Reported: ${new Date().toLocaleString()}</p>
          <p><a href="${process.env.APP_URL || 'http://localhost:8080'}/reports/${reportId}">View Details</a></p>
          <hr>
          <p style="color: #666; font-size: 12px;">This is an automated notification from the Disaster Management System.</p>
        `
      };
...
app.post("/reports", upload.single("image"), async (req, res) => {
  try {
    ...
    // Create alert for new report - this will send emails to all users
    await createAlert(
    ...
    req.flash("success", "Report submitted successfully! All users have been notified.");
    res.redirect("/reports");
  } catch (error) {
...
app.listen(PORT, () => {
  ...
  console.log(`Email notifications configured for: ${process.env.EMAIL_USERNAME}`);
});